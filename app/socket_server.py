...
from socket_service import emit_connected_computers, update_session, validate_tokens, is_valid_teacher_session, \
    ...
    create_users_checkpoints, create_log_for_users, create_log
...
def handle_connect(sid, environ):
    try:
        ...
        if not all([tokens_valid, user, computer_id]):
            sio.emit('errors', 'Второй токен должен быть пустым либо валидным, computer_id и первый Authorization обязательны')
            sio.disconnect(sid)
            return
...
def handle_disconnect(sid):
    try:
        ...
        create_log_for_users(sio=sio, endpoint="disconnect", computer_id=session[sid]["computer_id"], ids=session[sid]["ids"])
...
def start_events(sid, computers):
    try:
        ...
        create_log(sio=sio, endpoint="create_event", computer_id=session[sid]["computer_id"], id=session[sid]["ids"][0])
...
def checkpoint(sid, checkpoint_data: CheckpointData):
    try:
        ...
        create_log_for_users(sio=sio, endpoint=f"checkpoint {checkpoint_data['step']}", computer_id=session[sid]["computer_id"], ids=session[sid]["ids"])
