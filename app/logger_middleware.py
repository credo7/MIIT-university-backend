...
class CustomLogger:
    ...
    async def log(self, user_id=None, endpoint=None, status_code=None, request_method=None,
                  process_time=None,
                  computer_id=None,
        ...
        new_log = models.Log(user_id=getattr(user, 'id', None), endpoint=endpoint,
        ...
        if self.tg:
            ...
            request_line = f"{request_method} {endpoint} {status_code}" \
                if status_code and request_method and endpoint else 'Not defined'

            computer_id = computer_id if computer_id else 'Not defined'
            process_time = process_time if process_time else 'Not defined'

            message = f"🚀 {request_line}\n\n👤 " \
...
async def custom_logger_middleware(request: Request, call_next):
    ...
    if auth_header and auth_header.startswith("Bearer "):
        ...
        token = oauth2.verify_access_token(bearer_token, credentials_exception=None,
                                           raise_on_error=False)
    computer_id = request.headers.get("X-Computer-ID")
    await CustomLogger(tg=True).log(user_id=token.id if token else None,
                                    status_code=response.status_code,
                                    endpoint=request.url.path, request_method=request.method,
