...
from schemas import SelectLogist, EventType, EventMode, Incoterm, PR1ClassChosenBet, PR1ClassEvent, BetsRolePR1, \
    PR1ClassBetType, IncotermInfo, IncotermInfoSummarize
...
async def select_logist(
        select_logist_dto: SelectLogist,
    ...
    db[CollectionNames.EVENTS.value].update_one({
        ...
        {
            '$set': {
                'chosen_logist_index': select_logist_dto.logist_index
            }
        }
    )
...
@router.get(
    ...
    response_model=dict[Incoterm, IncotermInfoSummarize]
)
async def get_comparison_options(event_id: str, db: Database = Depends(get_db),):
    ...
    if pr1_class_event.delivery_options is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Запрашивались ли comparison-options до?"
        )
...
@router.get(
    '/comparison-options/',
    status_code=status.HTTP_200_OK,
    response_model=dict[Incoterm, IncotermInfo]
)
async def get_comparison_options(event_id: str, db: Database = Depends(get_db),):
    event_db = db[CollectionNames.EVENTS.value].find_one({
        "_id": event_id
    ...
    for incoterm in list(Incoterm):
        ...
        db[CollectionNames.EVENTS.value].update_one({'_id': ObjectId(pr1_class_event.id)},{
            "$set": {
                "delivery_options": incoterms_mapping_json
            }
        })
