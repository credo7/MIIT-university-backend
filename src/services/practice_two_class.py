...
class PracticeTwoClass:
    ...
    def checkpoint(self, event: Union[PR2ClassEvent, Type[PR2ClassEvent]], checkpoint_dto: CheckpointData):
        ...
        if checkpoint_dto.step_code == 'SCREEN_10_RISKS_3':
            ...
            event.risks_chosen_by_user.append(
                RisksWithRouteName(
                    route_name='Маршрут через Казахстан (через Достык/Алтынколь и Брест)',
                    risks=[risks_mapping[id] for id in checkpoint_dto.risk_ids_desc],
                )
            )
...
8.	Дать рекомендации по реализации вариантов с учетом рисков"""
    ...
    def _get_risks_step_response(event, route_index: int):
        ...
        route_names = [
            ...
            'Маршрут через Казахстан (через Достык/Алтынколь и Брест)',
    ...
    def _get_map_route_step_response(event, route_index):
        ...
        only_borders_text = 'последовательно выбирая пункт отправления, погранпереходы и пункт назначения на карте. Вы можете предварительно изучить объекты, кликнув по ним.'
        ports_and_borders_text = 'последовательно выбирая пункт отправления, порты, погранпереходы и пункт назначения на карте. Вы можете предварительно изучить объекты, кликнув по ним.'
